...
public class MyCalculator implements Calculator {
    ...
    private double ToCalculate(String expression)
        ...
        String[] PartsOfTheStringResult = expression.split(" ");  // разделяем строку по пробелам, на нулевой позиции пустая строка
        ...
        for (int i = 1; i < PartsOfTheStringResult.length; i++) {
            ...
            if (!IsOperator(part.charAt(0))) {  // Кладем в стек число
                NewStackResult.push(Double.parseDouble(part)); // преобразовуем строку в действительное число и добаляем в стэк
            } else  // а если это оператор, то считаем два  числа и результат гоняем в стэк
                if (part.length() == 1 && NewStackResult.size() >= 2 && part.charAt(0) != '$') {  // проверка что это оператор
                    double first = NewStackResult.pop(); // снимаем с вершины стэка первое слагаемое
                    double second = NewStackResult.pop(); // снимаем с вершины стэка второе слагаемое
                    NewStackResult.push(IntermediateCalculation(first, second, part.charAt(0))); // Делаем промежуточный расчет и кладем результат в стэк
                } else if (part.length() == 1 && NewStackResult.size() >= 1 && part.charAt(0) == '$') { // Если унарный минус
    ...
    private String GoToPolishNotation(String expression) throws ParsingException {
        ...
        boolean beginOfDoubleNubmer = false;
        for (Character chr : expression.toCharArray()) {
            ...
            if (IsNumber(chr)) {  // если число - добавляем в итоговую строку
                ...
                if (!beginningOfDoubleNumber) { // начало записи числа в строку
                    ...
                    if (chr == '.') { // Если это точка, то увиличиваем количество точек в данном числе
                        amounPoint++;
            ...
            if (IsOperator(chr)) { // проверка, что оператор
                if (flag) {  // проверка что унарный
                    ...
                    while (!StackOperations.empty()) { // Пока стэк операторов не пуст, выталкиваем из стэка всё символы, у которых приоритет больше данного и записываем в строку
                        Character PresentOperator = StackOperations.pop(); // Снимаем с вершины стэка
                        if (CheckPriority(chr) <= CheckPriority(PresentOperator)) { // сравниваем приоритет
                            StringResult.append(" ").append(PresentOperator);  // //записуем в результуюую строку
                        } else {
            ...
            if (chr.equals('(')) {  // если идет открыающиися скобка
                ...
                while (!StackOperations.empty()) { // то выталкиваем элементы из стека в итоговую строку
                ...
                PresenceOfOpeningBrackets = false;
            }
        }
    ...
    private boolean IsNumber(char chr)
        ...
        if (chr == '.' || chr == '0' || chr == '1' || chr == '2' || chr == '3' || chr == '4'
