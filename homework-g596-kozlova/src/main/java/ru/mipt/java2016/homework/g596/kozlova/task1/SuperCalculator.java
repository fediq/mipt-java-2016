...
public class SuperCalculator implements Calculator {
    ...
    private String getPolishNotation(String expression) throws ParsingException {
        // преобразование обычной записи в польскую - алгоритм из wikipedia
        boolean unaryCheck = true;
        // проверка, что следующий оператор действует над одним операндом, т.е. унарен
            ...
            if (ComponentsOfNumber.contains(c)) {
                ...
                if (unaryCheck) {
                    ...
                    if (c == '-' || c == '+') {
                        ...
                        throw new ParsingException("This is invalid expression");
                        // не смогли определить выражение
                    }
                } else {
                    // если оператор не унарен
                    result.append(' ');
                    unaryCheck = true;
                    while (!stack.empty()) {
                        /* вытаскиваем верхние элементы стека в строку, пока они есть и их приоритеты не меньше данного,
                ...
                /* если - закрывающая скобка, то пока верхним элементом стека не станет открывающая скобка,
                ...
                boolean startInStack = false;
                // проверка, что открывающая скобка в стеке уже есть
                while (!stack.empty()) {
                    // вытаскиваем верхние элементы стека в строку, пока они есть и мы не встретили открывающую скобку
                    Character tmp = stack.pop();
                    if (tmp == '(') {
                        // если мы нашли открывающую скобку - выходим из цикла
                        startInStack = true;
                        break;
                    } else {
                        result.append(' ').append(tmp).append(' ');
    ...
    private double calculateOfPolishNotation(String expression) throws ParsingException {
        // результат выражения, записанного в польской записи
        Scanner sc = new Scanner(expression);
        Stack<Double> stack = new Stack<>();
        // стек промежуточных результатов подсчета
        while (sc.hasNext()) {
            // перебор по всему выражению
            String s = sc.next();
            if (s.length() == 1 && s.charAt(0) == '&') {
                ...
                if (stack.size() > 1) {
                    ...
                    stack.push(result);
                    // подсчитанный результат отправляем в стек
                } else {
                    throw new ParsingException("This is invalid expression");
                }
