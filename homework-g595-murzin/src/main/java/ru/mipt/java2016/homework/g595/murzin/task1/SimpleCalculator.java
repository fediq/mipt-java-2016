	...
	private ArrayDeque<Token> convertToRPN(ArrayList<Token> input) throws ParsingException {
		...
		for (Token token : input) {
			if (token.type == TokenType.NUMBER) {
				...
				while (!stack.isEmpty() && stack.peekLast().isOperation() && priority <= ((TokenOperator) stack.peekLast()).priority) {
					output.addLast(stack.pollLast());
				}
				stack.addLast(token);
			}
		...
		for (Token token : rpn) {
			if (token.type == TokenType.NUMBER) {
				...
				if (stack.size() < operation.numberOfOperands()) {
					throw new ParsingException(String.format("Operator %s takes %d operands, only %d given", operation.type, operation.numberOfOperands(), stack.size()));
				}
