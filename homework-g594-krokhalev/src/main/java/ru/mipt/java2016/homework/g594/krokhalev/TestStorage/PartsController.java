...
public class PartsController<K, V> implements Closeable {
    private final int BASE_PART_SIZE;
    private final int LEVEL_INCREASE;
    ...
    public PartsController (@NotNull File workDirectory,
        ...
        if (basePartSize <= 0) {
            throw new RuntimeException("Minimal part size should be natural number");
        }

        if (levelIncrease <= 1) {
            throw new RuntimeException("Minimal part size should be more than 1");
        }

        BASE_PART_SIZE = basePartSize;
        LEVEL_INCREASE = levelIncrease;
