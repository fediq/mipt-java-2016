...
public class JMyCalculator implements Calculator {
    ...
    private void checkExpressionSymbolsAndBalance(String expression) throws ParsingException {
        int balance = 0;
        for (char C : expression.toCharArray()){
            if(!ACCEPTABLE_SYMBOLS.contains(C))
            ...
            if(isNumber(lexem))
            {
                ...
                switch (lexem) {
                    ...
                    case "-":
                        ...
                        else operators.add("?"); // "?" means unary minus
                        isNextUnary = true;
                        break;
                    case "+": {
                        if(!isNextUnary) {
                        ...
                        else operators.add("!"); // "!' means unary plus
                        isNextUnary = true;
                        break;
                    }
                    case "*": {
                        ...
                        while (!operators.empty() && (operators.peek().charAt(0) == '*' || operators.peek().charAt(0) == '/'))
                            result.add(operators.pop());
                        operators.add(lexem);
                        isNextUnary = true;
                        break;
