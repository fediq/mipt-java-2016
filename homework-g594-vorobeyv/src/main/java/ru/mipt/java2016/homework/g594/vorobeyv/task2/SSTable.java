...
public class SSTable<K, V> {
    ...
    private void Serialize( File data )throws IOException{
        DataOutputStream output = new DataOutputStream( new FileOutputStream( data ));
        if( closed ){
            data = new File( filePath );
        }
        int size = index.size();
    ...
    private void Desirialize( File data ) throws IOException {
        DataInputStream input = new DataInputStream( new FileInputStream( data ));
